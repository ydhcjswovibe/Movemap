...
`VITE_PUBLIC_SHARE_ORIGIN`은 외부 공유 링크에 사용할 공개 production origin입니다. 값 끝의 `/`는 앱에서 제거합니다. 이 값을 비워두면 현재 브라우저 origin으로 `/share/:id` 링크를 만듭니다.

Supabase SQL editor에서 테이블과 익명 정책을 만듭니다. 외부인이 로그인 없이 공유 링크를 열려면 `choreo_projects`에 anonymous select 정책이 반드시 있어야 합니다.
...
Supabase Storage에서 public bucket `choreo-audio`를 만듭니다. 음악 파일은 이 bucket에 저장되고 프로젝트 JSON에는 파일 자체가 아니라 `storagePath`, `publicUrl`, 파일명, 크기, fingerprint 같은 메타데이터만 저장됩니다.
...
음악은 `projects/{localProjectId}/audio/{fingerprint}-{fileName}` 경로에 저장됩니다. 같은 파일을 다시 선택하면 fingerprint로 기존 서버 음악을 다시 연결하므로 불필요한 중복 업로드를 줄입니다.
...
4. 음악이 있는 안무라면 재생 버튼으로 음악이 재생되는지 확인합니다.
...
- `안무 파일 공유`: 프로젝트 데이터를 안무 파일(.json)로 저장하고 나중에 `저장한 안무 열기`로 복원
